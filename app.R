...
server <- function(input, output, session){
  ...
  observeEvent(input$dbf2, {
    ...
    validate(need(!is.null(d2), "Falha ao ler o MENINNET2025.DBF enviado."))
    
    if ("NU_ANO" %in% names(d2)) {
      nu <- suppressWarnings(as.numeric(as.character(d2$NU_ANO)))
      d2 <- d2[!is.na(nu) & nu >= 2025, , drop = FALSE]
    } else if ("DT_NOTIFIC" %in% names(d2)) {
      d2 <- dplyr::filter(d2, lubridate::year(.parse_dt(DT_NOTIFIC)) >= 2025)
    }
    
    base25(d2)
    showNotification(
      sprintf("MENINNET2025.DBF carregado: %s registros com ano ≥ 2025.",
  ...
  dados_filtro <- reactive({
    d <- dados()
    if (!is.null(input$anos)) d <- d |> filter(ano %in% input$anos)
    if (!is.null(input$tipos) && length(input$tipos) > 0) {
      d <- d |> filter(TIPO_MENINGITE %in% input$tipos)
    }
    d
  })
  ...
  # ---------- Nova aba Variações: GRÁFICOS (fontes maiores + legenda) ----------
  output$plot_var_yoy_heat <- renderPlot({
    ...
    ggplot(yoy, aes(x = mes_nome, y = factor(ano), fill = variacao)) +
      ...
      scale_fill_gradient2(
        low = "#16a34a", mid = "#f8fafc", high = "#ef4444",   # mantém: vermelho=queda, verde=aumento
  ...
  # MoM (barras facetadas por ano) — vermelho = aumento; azul = queda; fontes maiores
  ...
  output$plot_var_mom_facets <- renderPlot({
    ...
    # MoM global: compara cada mês com o mês imediatamente anterior (inclusive jan vs. dez do ano anterior)
    mom <- d |>
      arrange(data) |>
      mutate(
        variacao = (Casos - dplyr::lag(Casos)) / dplyr::lag(Casos) * 100,
        lbl = ifelse(is.finite(variacao), paste0(sprintf('%.1f', variacao), '%'), '')
      )
    
    ggplot(mom, aes(x = mes_nome, y = factor(ano), fill = variacao)) +
      geom_tile(color = "white") +
      geom_text(aes(label = lbl), size = 5.0, fontface = "bold") +
      scale_fill_gradient2(
        low = "#16a34a", mid = "#f8fafc", high = "#ef4444",  # vermelho=queda, verde=aumento (igual aos outros)
        midpoint = 0, na.value = "#e5e7eb", name = "Variação (%)"
      ) +
      labs(
        title = "Variação % MoM por mês (heatmap)",
        x = "Mês", y = "Ano",
        caption = paste(
          "Como ler: cada célula compara o mês com o MÊS anterior (ex.: jan vs. dez do ano anterior).",
          "Ex.: −8% em 'set/2024' = set/2024 teve 8% menos casos que ago/2024.",
          "Cores: verde = queda; vermelho = aumento. Branco = sem base comparável."
        )
      ) +
